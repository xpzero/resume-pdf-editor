...
export function exportMarkdown(data) {
  const markdown = [
    ...
    `- 个人博客：${data.blog}`, `- 意向岗位：${data.role}`, '',
    '## 教育经历', ...data.educations.map(item => `- ${item.date}｜${item.school}｜${item.degree}｜${item.major}`), '',
    ...
    ...data.customSections.flatMap(section => [`## ${section.title || '未命名模块'}`, ...section.items.map(item => `- ${item}`), '']),
