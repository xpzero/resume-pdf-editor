...
export function WorkspaceHeader({ savedAt, previewOpen, onTogglePreview, onImport, onLlmConfig, onResumes, onOptimize, onExportMarkdown, onExportPdf }) {
  const exportMenu = { items: [{ key: 'markdown', label: '导出 Markdown' }], onClick: ({ key }) => { if (key === 'markdown') onExportMarkdown(); } };
  const moreMenu = { items: [{ key: 'import', label: '导入简历' }, { key: 'resumes', label: '简历列表' }, { key: 'llm', label: 'LLM 配置' }], onClick: ({ key }) => { if (key === 'import') onImport(); if (key === 'llm') onLlmConfig(); if (key === 'resumes') onResumes(); } };
