...
function Education({ data }) {
  return <ResumeSection title="教育经历">{data.educations.map((item, index) => <div className="education" key={index}><span>{item.date}</span><span>{item.school}</span><span>{item.degree}</span><span>{item.major}</span></div>)}</ResumeSection>;
}
...
function StandardDocument({ data, template }) {
  return <article className={`resume template-${template.id}`}>
    ...
    <Education data={data} /><Skills data={data} /><Experience data={data} /><Projects data={data} /><CustomSections data={data} /><Evaluation data={data} />
  </article>;
}
